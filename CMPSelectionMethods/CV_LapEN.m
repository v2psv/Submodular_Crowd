...
function [mse, mae] = CV_LapEN(type, Labeled, SemiFeature, SemiIdx, Ymean, Test, option)
	if nargin < 7
	...
	parfor i=1:length(strip_LambdaASet)
	    ...
	    for k = 1:nfold
	        ...
	        X = [Labeled.Feature(itrain,:); LapFeature];
	        Y = [Labeled.Label(itrain); zeros(size(LapFeature, 1), 1)];
