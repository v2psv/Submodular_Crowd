...
function [mse, mae] = CV_LapSSEN3(LabelFeature, LabelTruth, SemiFeature, SemiIdx, Ymean, Test, option)
	...
	parfor i=1:length(strip_LambdaReg)
		...
		LapSFeature = getLapFeature(SemiFeature, sigmaS);
	    ...
	    for k = 1:nfold
	        ...
	        X = [LabelFeature(itrain,:); LapSFeature; LapTFeature];
	        Y = [LabelTruth(itrain); zeros(size(LapSFeature, 1), 1); zeros(size(LapTFeature, 1), 1)];
	...
	% sigmaS = sqrt(exp(-option.K(SemiIdx, SemiIdx)/opt_widthS));
